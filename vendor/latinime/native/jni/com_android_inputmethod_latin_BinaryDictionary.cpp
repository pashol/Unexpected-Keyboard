...
static bool latinime_BinaryDictionary_validate(JNIEnv *env, jclass clazz, jlong dict) {
    Dictionary *dictionary = reinterpret_cast<Dictionary *>(dict);
    if (!dictionary) {
        return false;
    }
    const int probeCodePoint[] = { 'a' };
    dictionary->getProbability(CodePointArrayView(probeCodePoint, 1));
...
static const JNINativeMethod sMethods[] = {
    ...
    {
        const_cast<char *>("validateNative"),
        const_cast<char *>("(J)Z"),
        reinterpret_cast<void *>(latinime_BinaryDictionary_validate)
    },
    {
